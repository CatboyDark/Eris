...
export default 
{
    ...
    options: [
        {
            type: 'integer',
            name: 'days',
            desc: 'Filter GXP obtained in the last x days (Default: 7)'
        },
        {
            type: 'string',
            name: 'gxp',
            desc: 'Filter members by GXP below this threshold (\'50000\' or \'50k\')'
        },
        {
            type: 'integer',
            name: 'join_date',
    ...
    async execute(interaction) {
        ...
        const success = await interaction.followUp({
            embeds: [
                createMsg({
                    title: config.guild,
                    desc: '**Gathering data...**',
        ...
        const ignGxpPairs = gxp.map(
            (member) => `${member.ign.replace(/_/g, '\\_')} ${member.gxp}`
        );
