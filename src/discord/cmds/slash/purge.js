...
module.exports = {
    ...
    async execute(interaction)
    {
        ...
        if (count < 1)
        {
            return interaction.reply({ embeds: [minCount], ephemeral: true });
        }
        if (count > 100)
        {
            return interaction.reply({ embeds: [maxCount], ephemeral: true });
        }
        ...
        messages = messages.filter(
            (msg) => now - msg.createdTimestamp <= 1209600000
        );
        ...
        if (messages.size > 0)
        {
            ...
            const success =
                count === 1
                    ...
                    : createMsg({
                        desc: `**Deleted ${messages.size} ${filter === 'user' ? 'user' : filter === 'bot' ? 'bot' : ''} messages.**`
                    });
