...
module.exports = {
    ...
    async execute(interaction)
    {
        ...
        const validRoles = uniqueRoles.filter(
            (role) => !noPerms.includes(role)
        );
        ...
        const roleAdd = validRoles.filter(
            (role) => !user.roles.cache.has(role.id)
        );
        ...
        if (roleAdd.length > 0 || roleRemove.length > 0)
        {
            const addedRoles =
                roleAdd.length > 0
                    ? roleAdd.map((role) => `${plus} <@&${role.id}>`).join('\n')
                    : '';
            const removedRoles =
                roleRemove.length > 0
                    ? roleRemove
                        .map((role) => `${minus} <@&${role.id}>`)
                        .join('\n')
                    : '';

            const desc = [addedRoles, removedRoles]
                .filter(Boolean)
                .join('\n\n');
            const embed = createMsg({
                desc: `${user} **Updated roles!**\n\n${desc}`
            });
