...
const notLinked = createError(
    '**You are not linked! Please run /link to link your account!**'
);
...
module.exports = {
    ...
    async execute(interaction)
    {
        ...
        try
        {
                ...
                if (e.message.includes('Missing Permissions'))
                {
                    interaction.followUp({
                        embeds: [
                            createMsg({
                                color: 'FFD800',
                                desc: '**I don\'t have permission to change your nickname!**'
                            })
                        ]
                    });
                }
            ...
            const { addedRoles, removedRoles } = await updateRoles(
                interaction.member,
                player
            );
