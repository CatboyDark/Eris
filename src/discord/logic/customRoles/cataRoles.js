...
function createCataRolesMsg() {
	...
	const roleList = Object.entries(config.cataRoles).map(([level, roleID]) => `<@&${roleID}> - **Cata ${level}**`).join('\n');

	return createMsg({
		title: 'Custom Roles: Cata',
		desc: `You may assign a role to any Catacombs level.\n### Roles:\n${roleList}`
	});
}
...
function createRow2() {
	...
	const back = createRow([
		...
		{
			id: 'cataRolesToggle',
			label: 'Enable Cata Roles',
			style: config.features.cataRolesToggle
		}
	]);
