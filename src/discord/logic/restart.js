...
async function update(interaction)
{
    ...
    try
    {
        const [latestHashResult, localHashResult] = await Promise.all([
            axios.get(`${repoURL}/commits/main`, {
        ...
        if (currentHash === latestHash)
        {
            return interaction.followUp({
                embeds: [
                    createMsg({
                        desc: `** ${interaction.client.user.username} is already up to date!**`
                    })
                ],
                ephemeral: true
            });
        ...
        return interaction.followUp({
            embeds: [
                createMsg({
                    title: config.guild,
                    color: 'FF0000',
                    desc: '**Error checking for updates!**'
                })
            ]
        });
    }
    ...
    await interaction.followUp({
        embeds: [
            createMsg({
                color: '00FF00',
                desc: `**${interaction.client.user.username} has been updated!**`
            })
        ]
    });
}
