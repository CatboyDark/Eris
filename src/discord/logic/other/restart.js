...
async function update(interaction) {
  ...
  try {
    ...
    if (currentHash === latestHash) {
      return interaction.followUp({
        embeds: [createMsg({ desc: `** ${interaction.client.user.username} is already up to date!**` })],
        ephemeral: true
      });
    }
  } catch (error) {
  ...
  await interaction.followUp({
    embeds: [createMsg({ color: '00FF00', desc: `**${interaction.client.user.username} has been updated!**` })]
  });
}
