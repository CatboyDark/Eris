...
module.exports = [
    {
        ...
        async execute(interaction)
        {
            ...
            {
                const exceptionLogic = Object.keys(map).find((logic) =>
                    map[logic].includes(interaction.customId)
                );

                if (exceptionLogic)
                {
                    console.warn(
                        `Logic for ${interaction.customId} (${exceptionLogic}) does not exist!`
                    );
                }
                else
                {
                    console.warn(
                        `Logic for ${interaction.customId} does not exist!`
                    );
                }
            }
        }
