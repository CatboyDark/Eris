...
function readLogic() {
    ...
    function load(directory) {
        for (const file of fs.readdirSync(directory)) {
            ...
            if (stats.isDirectory()) load(filePath);
...
async function getCataHighest(player) {
    const sbMember = await hypixel.getSkyblockMember(player.uuid);
    let highestLevel = 0;
    for (const [profileName, profileData] of sbMember.entries()) if (highestLevel < profileData.dungeons.experience.level) highestLevel = profileData.dungeons.experience.level;
