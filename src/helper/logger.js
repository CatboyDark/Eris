...
async function buttonCounter(button, source) {
  await db.Button.findOneAndUpdate({ button }, { $inc: { count: 1 }, $set: { source } }, { upsert: true, new: true });
}
  ...
  switch (true) {
    case interaction.isChatInputCommand():
      if (config.logs.commands) {
        ...
        title = 'Command';
        desc = `<@${interaction.user.id}> ran **/${interaction.commandName}** ${
          optionsString
        }\n${interaction.message.url || 'Unknown URL'}`;
      } else {
        return null;
      }
      break;
    ...
    case interaction.isButton():
      if (config.logs.buttons) {
        title = 'Button';
        desc = `<@${interaction.user.id}> clicked **${
    ...
    case interaction.isStringSelectMenu():
      if (config.logs.menus) {
        ...
        title = 'Menu';
        desc = `<@${interaction.user.id}> selected **${optionLabels.join(', ')}** from **${
