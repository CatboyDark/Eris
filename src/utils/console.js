...
(function extendConsole() {
	...
	for (const [method, color] of Object.entries(colors)) {
		console[method] = function (title, error = null) {
			display(`^+${color}${title}`)
			if (error) console.log(error)
		}
	}

	console.error = async function (title, error = null) {
		console.red(title)
		if (error) console.log(error)
		...
		// 			desc:
		// 				'### A Silly has occured!\n' +
		// 				`\`\`\`${typeof error === 'string' ? error : error?.message ?? title}\`\`\`\n` +
...
export function debug(message, raw = null) {
	if (Config.debug) {
