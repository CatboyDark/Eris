...
export default {
	...
	async execute(message) {
		...
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played F7!`);
		}

		const pb = getPB(cata, 'floor7');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played F7!`);
		}
		...
		await message.reply(`${player.nickname}'s F7: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
