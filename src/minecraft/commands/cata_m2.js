...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.masterCatacombs.Floor_2;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played M2!`);
		}

		const pb = getPB(cata, 'masterCatacombs2');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played M2!`);
		}

		const collection = cata.completions.catacombs.Floor_2 + (cata.completions.masterCatacombs.Floor_2 * 2);

		await message.reply(`${player.nickname}'s M2: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
