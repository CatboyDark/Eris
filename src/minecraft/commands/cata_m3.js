...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.masterCatacombs.Floor_3;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played M3!`);
		}

		const pb = getPB(cata, 'masterCatacombs3');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played M3!`);
		}

		const collection = cata.completions.catacombs.Floor_3 + (cata.completions.masterCatacombs.Floor_3 * 2);

		await message.reply(`${player.nickname}'s M3: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
