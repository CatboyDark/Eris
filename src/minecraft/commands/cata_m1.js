...
export default {
	...
	async execute(message) {
		...
		if (message.options.profile === '-h') {
			cata = await getCata.highest(userData.id, 'm1').catch((e) => {
				console.log(e);
			});
		}
		else {
			cata = await getCata.current(userData.id, 'm1').catch((e) => {
				console.log(e);
			});
		}

		await message.reply(`${userData.ign}'s M1: Runs: ${cata.runs} | Total Collection: ${cata.collection} | PB: ${cata.score} ${cata.time}`);
	}
};
