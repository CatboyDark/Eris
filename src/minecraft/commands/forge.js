...
export default {
	...
	async execute(message) {
		...
		const response = await fetch(`https://api.hypixel.net/v2/skyblock/profiles?key=${auth.hypixelAPI}&uuid=${userData.id}`);
		const data = await response.json();
		const profile = data.profiles.find(profile => profile.selected);
		const player = profile.members[userData.id];

		const processes = player.forge.forge_processes.forge_1;
		if (Object.keys(processes).length === 0) {
			return message.reply(`${userData.ign}: Empty Forge!`);
		}
		...
		message.reply(`${userData.ign}'s Forge: ${list.join(' | ')}`);
	}
};
