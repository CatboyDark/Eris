...
export default {
	...
	async execute(message) {
		...
		message.reply(`${guild.name}: Level: ${Number(Math.floor(guild.level.toFixed(1)))} | GM: ${guildMaster.ign} | Members: ${guild.members.length}/125 | Weekly GXP: ${nFormat(guild.totalWeeklyGexp.toFixed(1))} `);
	}
};
