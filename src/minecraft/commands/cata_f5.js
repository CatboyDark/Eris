...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.catacombs.Floor_5;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played F5!`);
		}

		const pb = getPB(cata, 'floor5');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played F5!`);
		}
		...
		await message.reply(`${player.nickname}'s F5: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
