...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.catacombs.Floor_6;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played F6!`);
		}

		const pb = getPB(cata, 'floor6');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played F6!`);
		}
		...
		await message.reply(`${player.nickname}'s F6: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
