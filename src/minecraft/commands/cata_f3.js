...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.catacombs.Floor_3;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played F3!`);
		}

		const pb = getPB(cata, 'floor3');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played F3!`);
		}
		...
		await message.reply(`${player.nickname}'s F3: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
