...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.catacombs.Floor_2;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played F2!`);
		}

		const pb = getPB(cata, 'floor2');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played F2!`);
		}
		...
		await message.reply(`${player.nickname}'s F2: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
