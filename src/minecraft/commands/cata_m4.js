...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.masterCatacombs.Floor_4;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played M4!`);
		}

		const pb = getPB(cata, 'masterCatacombs4');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played M4!`);
		}
		...
		await message.reply(`${player.nickname}'s M4: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
