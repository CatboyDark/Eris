...
export default {
	...
	async execute(message) {
		...
		const runs = cata.completions.masterCatacombs.Floor_5;
		if (!runs) {
			return message.reply(`${player.nickname} hasn't played M5!`);
		}

		const pb = getPB(cata, 'masterCatacombs5');
		if (!pb) {
			return message.reply(`${player.nickname} hasn't played M5!`);
		}

		const collection = cata.completions.catacombs.Floor_5 + (cata.completions.masterCatacombs.Floor_5 * 2);

		await message.reply(`${player.nickname}'s M5: ${runs} Runs | PB ${pb.score}: ${pb.time} | Total Collection: ${collection}`);
	}
};
